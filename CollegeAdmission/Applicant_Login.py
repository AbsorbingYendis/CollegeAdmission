...
def login_action():
    ...
    cursor.execute('''
    SELECT * FROM applicant_login WHERE Username = ? AND Password = ?
    ...
    if user:
        if entered_code == verification_code:
            app.destroy()
            subprocess.run(["python", "user_dashboard.py", f"{user[0]}"])
        else:
...
label_title = tk.Label(frame, text="NSU Applicant Login", font=("Arial", 20, "bold"), bg="#e0f7fa")
